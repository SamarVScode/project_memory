...
The system maintains supply chain visibility across India's distribution network by updating granular transaction logs (`Raw`), hub-level daily summaries (`raw_task_1K`), and an operational tracking grid (`Task_per_1k`) that dynamically inserts 6-column daily metric tables in reverse chronological order while updating live Week-To-Date (`WTD`) formulas. Furthermore, it automatically provisions historical workbook links in `Past_reports_link` and builds three native Google Sheets pivot tables (`Task Category (FWD)`, `Task_Attribute`, `Task Category (REV)`) directly through script APIs. To eliminate timeout failures caused by Google Apps Script's hard 6-minute execution quota, the pipeline provides a dual execution model: an unattended 7:00 AM daily scheduled trigger (`Main.js:61-79`) and a client-orchestrated, chunked backfill Web Application (`WebApp.js:11-15`, `Index.html:1-395`) that processes historical multi-date ranges sequentially across independent HTTP Remote Procedure Call (RPC) threads.
